...
// ==================== SERVER API FUNKSIYALAR ====================

async function saveDataToServer() {
  try {
    const response = await fetch(`${API_BASE}/api/save-challenge-data`, {
      method: "POST",
    ...
    console.error("Server ulanishida muammo:", error);
    // Offline holatida ham davom etsin
  }
}

async function loadDataFromServer() {
  try {
    const response = await fetch(`${API_BASE}/api/get-challenge-data`);
    ...
    if (data && data.participants && data.participants.length > 0) {
      participants = data.participants.map((p) => ({
        ...
        lastShieldUpdate:
          p.lastShieldUpdate !== undefined ? p.lastShieldUpdate : null,
      }));
    ...
    console.error("Serverni yuklashda muammo:", error);
  }
  return false;
}

// ==================== PASSWORD VA SECURITY ====================
...
function updateShieldsAuto() {
  ...
  participants.forEach((p) => {
    if (!p.lastShieldUpdate) {
      p.lastShieldUpdate = endTime - CHALLENGE_DURATION;
    }
...
window.subtract = function (id) {
  ...
  if (!p || (p.nextAllowedTime && now < p.nextAllowedTime) || p.score <= 0)
    return;
  ...
  askPassword(() => {
    ...
    if (p.shields > 0) {
      ...
      try {
        new Audio("./ovozlar/shield.MP3").play();
      } catch (e) {}
      saveDataToServer();
      renderUI();
      renderLogs();
...
submitReasonBtn.onclick = function () {
  ...
  if (p) {
    ...
    const randomExercise =
      EXERCISE_POOL[Math.floor(Math.random() * EXERCISE_POOL.length)];
...
function renderUI() {
  ...
  let kings = participants
    .filter((p) => p.score === maxScore && p.score > 0)
    .map((p) => p.name.split(" ")[0]);
  document.getElementById("stat-king").innerText =
    kings.length > 0 ? kings.join(", ") : "Hech kim";

  let dangerOnes = participants
    .filter((p) => p.score <= 5 && p.score > 0)
    .map((p) => p.name.split(" ")[0]);
  document.getElementById("stat-danger").innerText =
    dangerOnes.length > 0 ? dangerOnes.join(", ") : "Yo'q";
  ...
  participants.forEach((p) => {
    ...
    const shieldDisplay =
      p.shields > 0
        ? `<div class="shield-box" style="position:absolute; top:10px; right:10px; background:#1e90ff; padding:2px 6px; border-radius:4px; font-size:12px; color:#fff;">🛡️ x${p.shields}</div>`
        : "";
...
setInterval(async () => {
  try {
    ...
    if (serverData && serverData.participants) {
      // Agar server ma'lumotlari farqli bo'lsa, yangilash
      const serverParticipantsStr = JSON.stringify(serverData.participants);
      const localParticipantsStr = JSON.stringify(participants);

      if (serverParticipantsStr !== localParticipantsStr) {
        participants = serverData.participants.map((p) => ({
          ...p,
          exercises: p.exercises !== undefined ? p.exercises : [],
          shields: p.shields !== undefined ? p.shields : 0,
          lastShieldUpdate:
        ...
        if (serverData.endTime !== endTime) {
          endTime = serverData.endTime;
          if (endTime && !timerInterval) {
            startTimer();
          }
        }
        ...
        renderMoney();
        updateGorillaMoneyUI();
        console.log("📡 Server ma'lumotlari yangilandi");
      }
    }
  } catch (error) {
    // Offline holatida muammo yo'q
  }
...
// Sahifa yuklanayotganda ma'lumotlarni yuklash
